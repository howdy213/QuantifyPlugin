...
bool ClassRule::analyzeRecordFile(const QString &file, const QString &date) {
    QFile f(m_cr->folder + "/record/" + date + ".record");
    if (!f.open(QIODevice::ReadOnly)) {
        Logger::instance().error("无法打开记录文件: " + date + ".record");
        return false;
    }
    ...
    // 解密（如果需要）
    QByteArray plainData = Encryptor::decryptData(rawData);
    if (plainData.isEmpty() && !rawData.isEmpty()) {
        // 解密失败且不是空文件
        Logger::instance().error("解密记录文件失败: " + date + ".record");
        return false;
    }
    QString content = QString::fromUtf8(plainData);
    if (content.isEmpty() && !plainData.isEmpty()) {
        Logger::instance().error("记录文件编码错误: " + date + ".record");
        return false;
    }
    content.remove('\r');
    ...
    if (lines[0] == "daily")
        ...
        rt = TERMLY;
    if (rt == NONE) {
        Logger::instance().error(date + ":未找到日期类型");
        return false;
    }
...
void ClassRule::changeRecord(const QStringList &param, const QString &date,
    ...
    if (reason == "custom")
        ...
        ExecuteResult result = rule->result(rt, old);
        if (!result.log.message.isEmpty()) {
            Logger::instance().info(date + " " + stuIt.key() + " " +
                                    result.log.message); // 以后支持更多类型
        }
        it.value() = result.record;
    }
