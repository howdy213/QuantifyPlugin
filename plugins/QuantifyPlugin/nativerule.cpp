...
bool NativeRule::loadFromFile(const QString &content) {
    ...
    if (lines.isEmpty()) {
        Logger::instance().error("规则文件为空");
        return false;
    }
    const auto first = lines[0].split(' ', Qt::SkipEmptyParts);
    if (first.isEmpty()) {
        Logger::instance().error("规则文件第一行格式错误：需要英文名 中文名");
        return false;
    }
    if (first.isEmpty())
        return false;
...
/// \return
///
ExecuteResult NativeRule::result(RecordType rt, const Record &rec) const {
    int idx = -1;
    switch (rt) {
    case DAILY:
        idx = 0;
